...
const createProduct = async (product) => {
    ...
    const result = await client.query(query, [product_name, product_price, product_quantity, description]);
    return result.rows[0];
};
...
const updateProduct = async (id, product) => {
    ...
    const result = await client.query(query, [product_name, product_price, product_quantity, description, id]);
    return result.rows[0];
};
